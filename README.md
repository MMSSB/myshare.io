...
PeerFlow is a high-performance, responsive Peer-to-Peer (P2P) file-sharing application designed for the modern web. Built with WebRTC, it allows you to transfer files directly between devices without the need for a backend server, making it the perfect tool to host for free on GitHub Pages.

## 🚀 Key Features

* **Zero-Backend Architecture:** 100% static files. No databases, no server costs, no file size limits (browser-dependent).
...
* **Rich History:** Automatic, persistent tracking of every sent and received file, including timestamps and file sizes.
* **Cross-Platform Ready:** Auto-detects device types (Mobile, Laptop, Desktop) and uses appropriate UI icons.
* **Secure & Stable:** Robust reconnection logic, connection state management, and password-optional security features.
...
5. **Manage:** Right-click (desktop) or Long-press (mobile) on any paired device card to rename, reconnect, or delete them.
...
## 🛡 Security
PeerFlow prioritizes privacy. Because it uses WebRTC, the connection is end-to-end encrypted. We do not store, track, or intercept your files.

---

*Built with passion. Inspired by the native sharing experiences of Apple and the modern web.*
